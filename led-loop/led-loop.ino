...
void loop()                     // run over and over again
{
  ...
  int ledPinAmount = 8;
  int delayTime = 500;
  int cycleLen = ledPinAmount + tailLen;

  // there and back again, in one cycle
  for(int i = 0; i <= cycleLen * 2; i++) {

    bool dir = i < cycleLen;

    if(i == cycleLen) delay(100);
    
    int idx = dir ? i:cycleLen * 2 - tailLen - i;
    int dimIdx = idx + (!dir ? tailLen:-tailLen);
    
    bool tailFilled = dir ? idx + 1 > tailLen : ledPinAmount - idx > tailLen;    
    bool inLedRange = idx < ledPinAmount && idx >= 0;
    
    if(inLedRange) digitalWrite(ledPins[idx], HIGH);    
